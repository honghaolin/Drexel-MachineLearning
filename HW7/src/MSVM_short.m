...
model = cell( (index_max-1)*index_max/2 , 1);
index = 1; % mark position for model saving
for i = 1 : index_max - 1
    for j = i + 1 : index_max 
        % get Training data
        data = data_training(...
            data_training(:, end) == i | data_training(:, end) == j , :);
        % train the SVM model
        model{index} = fitcsvm(data(:, 1:end-1), data(:, end));
...
% clean temp variables
clear index_max index i j data;
